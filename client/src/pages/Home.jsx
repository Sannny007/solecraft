    ...
    <div>
      <section className="relative px-6 pt-16 pb-20 md:pt-24 md:pb-28 overflow-hidden">
        <div ref={heroRef} className="max-w-4xl mx-auto text-center relative z-10">
          <span className="inline-block text-xs tracking-[0.3em] text-(--accent) font-semibold mb-4">
            DESIGN &middot; CUSTOMIZE &middot; WEAR
          </span>
          <h1 className="font-display text-5xl sm:text-6xl md:text-7xl leading-[0.95] mb-6">
            BUILD A SNEAKER<br />
            <span className="text-(--accent)">THAT'S YOURS</span>
          </h1>
          <p className="text-(--ink-dim) text-base md:text-lg max-w-xl mx-auto mb-8">
        ...
        <div className="absolute top-10 left-[8%] w-24 h-24 rounded-full bg-(--accent)/10 blur-2xl animate-float" />
        <div className="absolute bottom-0 right-[10%] w-32 h-32 rounded-full bg-(--accent-2)/10 blur-2xl animate-float" style={{ animationDelay: '1.2s' }} />
      </section>
      <section id="products" className="px-6 pb-20 max-w-7xl mx-auto">
        <div className="flex items-end justify-between mb-8 flex-wrap gap-3">
          ...
          <span className="text-(--ink-dim) text-sm">{items.length} models available</span>
        </div>
        {loading && <Spinner />}
        {error && <p className="text-(--accent-2)">{error}</p>}
          ...
          {items.map((product) => (
            <Link key={product._id} to={`/product/${product._id}`} className="card group block">
              ...
              <div className="relative h-52 overflow-hidden">
                ...
                <div className="absolute top-3 right-3 bg-(--accent) text-(--accent-ink) p-2.5 rounded-full opacity-0 group-hover:opacity-100 transition-all duration-300 translate-y-2 group-hover:translate-y-0">
                  <ShoppingCart size={16} />
                </div>
              </div>
              <div className="p-5">
                <h3 className="font-display text-xl mb-1 tracking-wide">{product.name}</h3>
                <p className="text-(--ink-dim) text-sm mb-3 line-clamp-2">{product.description}</p>
